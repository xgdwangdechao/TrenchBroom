...
namespace TrenchBroom {
    namespace View {
        ...
        bool MapDocumentCommandFacade::performTransform(const vm::mat4x4 &transform, const bool lockTextures) {
          ...
          if (visitor.error()) {
              error() << "Could not transform objects: " << visitor.error()->msg();
              return false;
          } else {
              return true;
          }
        ...
        std::optional<std::vector<vm::polygon3>> MapDocumentCommandFacade::performResizeBrushes(const std::vector<vm::polygon3>& polygons, const vm::vec3& delta) {
            ...
            for (Model::BrushNode* brushNode : selectedBrushes) {
                ...
                auto moveResult = original.moveBoundary(m_worldBounds, *faceIndex, delta, pref(Preferences::TextureLock));
                const bool success = kdl::visit_result(kdl::overload {
                    ...
                    [&](const GeometryException& e) -> bool {
