...
namespace TrenchBroom {
    namespace View {
        ...
        TEST_CASE_METHOD(SelectionTest, "SelectionTest.selectTouchingWithGroup") {
            ...
            Model::BrushNode* brush = new Model::BrushNode(kdl::get_success(builder.createCuboid(brushBounds, "texture")));
            document->addNode(brush, group);

            const vm::bbox3 selectionBounds(vm::vec3(-16.0, -16.0, -48.0),
                                        vm::vec3(+16.0, +16.0, +48.0));

            Model::BrushNode* selectionBrush = new Model::BrushNode(kdl::get_success(builder.createCuboid(selectionBounds, "texture")));
